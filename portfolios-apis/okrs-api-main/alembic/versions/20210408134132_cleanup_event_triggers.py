"""cleanup_event_triggers

Revision ID: 20210408134132
Revises: 20210407121533
Create Date: 2021-04-08 13:41:34.689015
...
revision = "20210408134132"
down_revision = "20210407121533"
branch_labels = None
depends_on = None
...
HASURA_METADATA_TABLE_FILE = Path(
...
def make_trigger_group(trigger_dict):
    ...
    return [f"{name}_{op.upper()}" for op in found_ops]
...
def convert_hasura_triggers(triggers):
    ...
    trigger_groups = [make_trigger_group(trigger_dict) for trigger_dict in triggers]

    # flatten the list
    return [name for group in trigger_groups for name in group]


with open(HASURA_METADATA_TABLE_FILE, "r") as stream:
    try:
        HASURA_METADATA_TABLES = yaml.safe_load(stream)
...
HASURA_METADATA = []
for table_name_path in HASURA_METADATA_TABLES:
    table_name = table_name_path.split(" ")[-1]
    path = f"hasura/metadata/databases/default/tables/{table_name}"
    with open(path, "r") as stream:
        try:
            table_obj = yaml.safe_load(stream)
            HASURA_METADATA.append(table_obj)
        except yaml.YAMLError as exc:
            print(exc)


CURRENT_PG_TRIGGERS = {
    table_obj["table"]["name"]: convert_hasura_triggers(
        table_obj.get("event_triggers", [])
    )
...
def upgrade():
    ...
    #             print(
    #                 f"!! trigger ({trigger_name}) in {table_name} is unused. DELETING..."
    #             )
    #             op.execute(
