...
Revision ID: 20231122173205
Revises: 20231031171319
Create Date: 2023-11-22 17:32:07.832668
...
revision = "20231122173205"
down_revision = "20231031171319"
branch_labels = None
depends_on = None
...
def upgrade():
 ...
 $$;
    ...
    # Drop and Recreate the trigger `platforma_set_timestamp_trigger` on `custom_attributes_settings`
    op.execute(
        """
    CREATE TRIGGER platforma_set_timestamp_trigger
    BEFORE INSERT OR UPDATE ON custom_attributes_settings
    FOR EACH ROW
    EXECUTE PROCEDURE platforma_functions.platforma_set_timestamp_func();
    ALTER TABLE custom_attributes_settings ENABLE TRIGGER platforma_set_timestamp_trigger;
    """
    )

    # Drop and Recreate the trigger `platforma_set_timestamp_trigger` on `custom_attributes_configs`
    op.execute(
        """
    CREATE TRIGGER platforma_set_timestamp_trigger
    BEFORE INSERT OR UPDATE ON custom_attributes_configs
    FOR EACH ROW
    EXECUTE PROCEDURE platforma_functions.platforma_set_timestamp_func();
    ALTER TABLE custom_attributes_configs ENABLE TRIGGER platforma_set_timestamp_trigger;
    """
    )

    # Drop and Recreate the trigger `platforma_set_timestamp_trigger` on `custom_attributes_values`
    op.execute(
        """
    CREATE TRIGGER platforma_set_timestamp_trigger
    BEFORE INSERT OR UPDATE ON custom_attributes_values
    FOR EACH ROW
    EXECUTE PROCEDURE platforma_functions.platforma_set_timestamp_func();
    ALTER TABLE custom_attributes_values ENABLE TRIGGER platforma_set_timestamp_trigger;
    """
    )
...
BEGIN
    ...
    NEW.level_depth_default = COALESCE(default_level_depth::int, 3);
    NEW.objective_editing_levels = CONCAT('[', COALESCE(default_level_depth, '3'), ']');
    RETURN NEW;
END;
 $$;
    """
